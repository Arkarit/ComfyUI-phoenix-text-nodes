...
- **PhoenixRandomCSVTextReplace** — replaces consecutive placeholders ($1, $2, ...) in a text with a randomly chosen term from a CSV candidate list (one line per placeholder). Ported from [comfyui-text-placeholder-randomizer](https://github.com/Arkarit/comfyui-text-placeholder-randomizer), whose `RandomCSVTextReplace` node continues to exist independently.
  ...
  - A line whose first non-blank character is a `,` is a **continuation**: it is appended to the last preceding line that had content, so one long row can be split over several source lines for readability. Blank lines and `#` comment lines in between do not break the continuation. Only a *leading* comma continues a row — a trailing comma deliberately does not, so a forgotten comma can't silently swallow the next row.
  ...
  - A candidate can carry `_DEFINE("name")_` to define a variable when it is picked, and `_IF("name")_` to require its presence or `_IFNOT("name")_` to require its absence. Variables travel along the node's optional `defines` input/output sockets, so the dependency is an explicit link (which also keeps ComfyUI's execution order and caching correct); within one node, later rows already see what earlier rows defined. The `pass_through_defines` widget (default on) controls whether incoming names are forwarded to the output. Gating is exclusive: if any candidate in a row matches, only the matching ones are eligible; if none does, only the candidates without any `_IF(...)_` or `_IFNOT(...)_`. A row left with nothing to pick behaves like `_NONE_`, keeping its placeholder index. Names inside one tag are ANDed (`_IFNOT(a b)_` requires both to be absent), separate condition tags ORed, including mixed `_IF`/`_IFNOT` tags. Works together with `_NODE(...)_` — the pre-queue pass follows the same `defines` chain.
...
- **PhoenixLoadLora** — applies a LoRA to a `model_clip_text` struct's `model` (and `clip`, if set), like the built-in LoRA loader, but also reads that LoRA's trigger keywords and accumulates them (newline-joined) into the struct's `keywords`. `model_clip_text` is both its only input and its only output — chain multiple LoRAs by feeding one's output into the next one's input, with a single link. Since input and output are the same type, bypassing any LoRA in the middle of a chain works correctly out of the box (ComfyUI's node-bypass forwards a same-type input straight to the matching output).
  - Keywords are looked up two ways, combined (deduplicated): (1) `loras_tags.json` (ComfyUI root, `{"filename.safetensors": ["keyword", ...]}`, one entry per line — matched by the `lora_name` combo value, falling back to its basename for a subfolder mismatch); (2) the LoRA file's own embedded training metadata, read directly from its `.safetensors` header (no full weight load) — `modelspec.trigger_phrase` if present, plus the concept name(s) from Kohya's `ss_tag_frequency` (its `"<repeats>_<concept>"`-named training folders). A LoRA with neither simply contributes no keywords, not an error.
  - Use **PhoenixModelClipTextPack** (see below) to produce the initial struct at the head of a chain (from a checkpoint loader's `model`/`clip`), and **PhoenixModelClipTextUnpack** to split the final struct back into `model`/`clip`/`keywords` at the end.
  - If the struct's `model` is unset and `strength_model` isn't 0, the node raises a clear error instead of failing obscurely.
- **PhoenixModelClipTextPack** — packs `model`, `clip` (both optional) and an optional starting `keywords` text into a `model_clip_text` struct (its only output), without applying any LoRA. Use it at the head of a **PhoenixLoadLora** chain; `keywords` also lets you seed the accumulated text with a base prompt before any LoRA's own keywords are added.
- **PhoenixModelClipTextUnpack** — the inverse: takes a `model_clip_text` struct (its only input) and outputs `model`, `clip` and `keywords` separately again, e.g. at the end of a **PhoenixLoadLora** chain. `keywords` is ready to feed directly into a `CLIP Text Encode` or text node; if non-empty, it gets one trailing newline appended after the last entry.
