...
- **PhoenixRandomCSVTextReplace** — replaces consecutive placeholders ($1, $2, ...) in a text with a randomly chosen term from a CSV candidate list (one line per placeholder). Ported from [comfyui-text-placeholder-randomizer](https://github.com/Arkarit/comfyui-text-placeholder-randomizer), whose `RandomCSVTextReplace` node continues to exist independently.
  ...
  - A candidate can carry `_DEFINE("name")_` to define a variable when it is picked, and `_IF("name")_` to be gated on one. Variables travel along the node's optional `defines` input/output sockets, so the dependency is an explicit link (which also keeps ComfyUI's execution order and caching correct); within one node, later rows already see what earlier rows defined. The `pass_through_defines` widget (default on) controls whether incoming names are forwarded to the output. Gating is exclusive: if any candidate in a row matches, only the matching ones are eligible; if none does, only the candidates without any `_IF(...)_`. A row left with nothing to pick behaves like `_NONE_`, keeping its placeholder index. Names inside one tag are ANDed, separate tags ORed. Works together with `_NODE(...)_` — the pre-queue pass follows the same `defines` chain.
